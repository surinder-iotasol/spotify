...
Story ID: STORY-profile-008a
Title: Scaffold Listener Profile page route
Description: Create src/pages/ListenerProfileView page with routing for /users/[userId]. Fetch user data via the existing GET /api/v1/users/:userId endpoint and display loading and error states.
Acceptance Criteria:
- Page renders at /users/[userId] with loading skeleton while data fetches.
- Error state displays when API returns a non-200 response.
- Unit test in src/pages/ListenerProfileView.test.tsx verifies page renders with mock user data.
...
5) Verify with: jq -r --arg id "STORY-profile-008a" '.userStories[] | select(.id == $id) | .passes' prd.json
