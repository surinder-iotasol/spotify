...
Story ID: STORY-profile-003d
Title: Add unit tests for profile image upload service
Description: Write unit tests covering file type validation, size checking, and storage provider invocation for both avatar and header upload flows.
Acceptance Criteria:
- Tests cover valid JPEG, PNG, and WebP MIME type acceptance.
- Tests reject non-image MIME types with correct 422 error responses.
- Tests enforce 5MB size limit and reject oversized files.
- Tests mock StorageProvider to verify upload invocation and URL capture.
...
5) Verify with: jq -r --arg id "STORY-profile-003d" '.userStories[] | select(.id == $id) | .passes' prd.json
