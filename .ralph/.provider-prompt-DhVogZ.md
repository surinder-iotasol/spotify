...
Story ID: STORY-profile-008c
Title: Build public playlists grid component
Description: Create a playlists grid that fetches and displays public playlists for a listener with cover mosaics, titles, and track counts.
Acceptance Criteria:
- Grid renders playlist cards fetched via the listener profile API for playlists where isPublic is true.
- Each card shows a cover mosaic image, playlist title, and track count.
- Card is clickable and navigates to the playlist detail page on click.
...
5) Verify with: jq -r --arg id "STORY-profile-008c" '.userStories[] | select(.id == $id) | .passes' prd.json
