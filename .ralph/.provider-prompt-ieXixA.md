...
Story ID: STORY-profile-008d
Title: Add followed artists carousel
Description: Build a horizontal carousel component showing followed artist cards with avatars and names, supporting scroll and keyboard navigation.
Acceptance Criteria:
- Carousel renders followed artists fetched from the listener profile API.
- Artists display in a horizontally scrollable container with visible scroll indicators.
- Users can navigate the carousel with arrow keys and Tab.
...
5) Verify with: jq -r --arg id "STORY-profile-008d" '.userStories[] | select(.id == $id) | .passes' prd.json
