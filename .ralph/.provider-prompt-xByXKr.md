...
Story ID: STORY-profile-003b
Title: Implement avatar upload endpoint
Description: Build POST /api/v1/artist-profile/avatar endpoint that uploads image to storage and updates the artist profile avatarUrl field.
Acceptance Criteria:
- Endpoint accepts authenticated ARTIST role requests with multipart image.
- Uploads validated image to cloud storage via StorageProvider interface.
- Updates ArtistProfile record with returned HTTPS asset URL in avatarUrl field.
- Returns updated profile payload in JSON envelope with HTTP 200 status.
...
5) Verify with: jq -r --arg id "STORY-profile-003b" '.userStories[] | select(.id == $id) | .passes' prd.json
