...
Story ID: STORY-track-002bb
Title: Calculate WAV duration from data chunk
Description: Calculate precise float duration in seconds from data chunk size, sample rate, channels, and bits per sample.
Acceptance Criteria:
- calculates duration in float seconds from data chunk size and format params
- handles both 8-bit and 16-bit PCM correctly with proper bytes-per-sample divisor
- throws CorruptedAudioError when data chunk is missing from WAV file
...
5) Verify with: jq -r --arg id "STORY-track-002bb" '.userStories[] | select(.id == $id) | .passes' prd.json
