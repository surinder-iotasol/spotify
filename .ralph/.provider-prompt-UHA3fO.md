...
Story ID: STORY-track-002bc
Title: Add WAV header parsing tests
Description: Unit tests for WAV header parsing covering valid files, edge cases, and error conditions.
Acceptance Criteria:
- tests valid WAV with known sample rate, channels, and duration match expected values
- tests error cases: missing fmt chunk, missing data chunk, non-PCM format
- tests boundary values: mono vs stereo, 8-bit vs 16-bit vs 24-bit PCM
...
5) Verify with: jq -r --arg id "STORY-track-002bc" '.userStories[] | select(.id == $id) | .passes' prd.json
