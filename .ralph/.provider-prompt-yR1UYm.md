...
Story ID: STORY-track-002a
Title: Create CorruptedAudioError class and metadata type definitions
Description: Define the CorruptedAudioError typed error class and the AudioMetadataInfo type used by all downstream header parsers.
Acceptance Criteria:
- src/services/audioMetadata.ts exports CorruptedAudioError class extending Error with fileKey and reason fields.
- src/services/audioMetadata.ts exports AudioMetadataInfo type with duration, bitrate, sampleRate, channels, codec, and durationMs fields.
...
5) Verify with: jq -r --arg id "STORY-track-002a" '.userStories[] | select(.id == $id) | .passes' prd.json
