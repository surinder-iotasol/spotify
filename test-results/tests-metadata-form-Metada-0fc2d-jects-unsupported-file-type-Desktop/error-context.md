...
- Name: tests/metadata-form.spec.ts >> MetadataEntryForm >> cover art picker rejects unsupported file type
- Location: e2e/tests/metadata-form.spec.ts:127:7

# Error details

```
Error: locator.click: Error: strict mode violation: getByText('Hip Hop') resolved to 2 elements:
    1) <option value="HIP_HOP">Hip Hop</option> aka locator('select')
    2) <span id="radix-:rd:">Hip Hop</span> aka getByLabel('Hip Hop').getByText('Hip Hop')

Call log:
  - waiting for getByText('Hip Hop')
